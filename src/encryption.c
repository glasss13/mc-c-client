...
void enable_encryption(struct Connection* conn, unsigned char* shared_secret) {
    ...
    EVP_EncryptInit_ex(conn->encryption_ctx, EVP_aes_128_cfb8(), NULL, shared_secret, shared_secret);
    EVP_DecryptInit_ex(conn->decryption_ctx, EVP_aes_128_cfb8(), NULL, shared_secret, shared_secret);
}

int aes_encrypt(EVP_CIPHER_CTX* ctx, const unsigned char* inp, unsigned char* outbuf, int in_len) {
...
int aes_decrypt(EVP_CIPHER_CTX* ctx, const unsigned char* inp, unsigned char* outbuf, int in_len) {
    unsigned char* outbuf_off = outbuf;
    int out_len;
    for (int i = 0; i < in_len; i++) {
