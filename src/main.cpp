    ...
    for (;;) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (bot.conn.encryption_enabled) {
                aes_decrypt(bot.conn.decryption_ctx, &current_byte, &current_byte, 1);
            }
        ...
        if (bot.conn.encryption_enabled) {
            ...
            aes_decrypt(bot.conn.decryption_ctx, (unsigned char*)bot.conn.inbuffer, temp_out, packet_length);
            memcpy(bot.conn.inbuffer, temp_out, sizeof(temp_out));
        }
        
        if (bot.conn.compthresh > -1) { // decompress
            int data_length = decode_varint(bot.conn.inbuffer); // Length of uncompressed (Packet ID + Data) or 0 if compression isn't enabled
            if (data_length == 0) { // if compression isn't enabled, get rid of data length varint stored in buffer and then continue as normal
                memcpy(bot.conn.inbuffer, bot.conn.inbuffer + ivarint_size(data_length), sizeof(bot.conn.inbuffer) - ivarint_size(data_length));
            } else {
                zdecompress(packet_length - ivarint_size(data_length), bot.conn.inbuffer + ivarint_size(data_length), data_length, bot.conn.inbuffer);
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    struct Packet handshake_packet = {HANDSHAKE, .data.C00Handshake = {47, ip, port, LOGIN}};
