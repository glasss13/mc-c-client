...
int write_C00Handshake(struct Packet* packet, char* buffer) {
    ...
    int length = varint_size(protocol) + pstrsize(addr) + sizeof(port) + varint_size(state) + sizeof(packet->id);
...
int write_C01PacketEncryptionResponse(struct Packet* packet, char* buffer) {
    varint secret_len = encode_varint(packet->data.C01PacketEncryptionResponse.ss_len);
    varint verify_len = encode_varint(packet->data.C01PacketEncryptionResponse.vt_len);

    int length = varint_size(secret_len) + varint_size(verify_len) + packet->data.C01PacketEncryptionResponse.ss_len + packet->data.C01PacketEncryptionResponse.vt_len + sizeof(packet->id);
    varint vlength = encode_varint(length);
    ...
    memcpy(temp_offset, packet->data.C01PacketEncryptionResponse.ss, packet->data.C01PacketEncryptionResponse.ss_len);
    ...
    memcpy(temp_offset, packet->data.C01PacketEncryptionResponse.vt, packet->data.C01PacketEncryptionResponse.vt_len);
...
void send_packet(struct Packet* packet, struct Connection* conn) {
    ...
    if (conn->state == HANDSHAKING) {
        ...
        switch(packet->id) {
            case CHAT_MESSAGE_S:
                packet_length = write_C01PacketChatMessage(packet, conn->outbuffer);
                break;
            default:
        ...
        switch(packet->id) {
            case LOGIN_START:
                packet_length = write_C00PacketLoginStart(packet, conn->outbuffer);
                break;
            case ENCRYPTION_RESPONSE:
                packet_length = write_C01PacketEncryptionResponse(packet, conn->outbuffer);
                break;
            default:
    ...
    if (conn->compthresh > -1) { // compression is enabled
        ...
        if (packet_length < conn->compthresh) { // don't compress this packet
            ...
            // change format packet_length | data to packet_length | data_length | data. memmove shifts over data to accomadate the comp_data_length
            memmove(conn->outbuffer + ivarint_size(data_length) + ivarint_size(packet_length), conn->outbuffer + ivarint_size(packet_length), sizeof(conn->outbuffer) - ivarint_size(packet_length));
            // copy data_length to the open memory given by the memmove
            memcpy(conn->outbuffer + ivarint_size(packet_length), vdata_length, varint_size(vdata_length));
            free(vdata_length);

            varint vpacket_length = encode_varint(packet_length);
            // overwrite length
            memcpy(conn->outbuffer, vpacket_length, varint_size(vpacket_length));
            free(vpacket_length);
            ...
            int compressed_length = zcompress(packet_length, conn->outbuffer + ivarint_size(packet_length), sizeof(conn->outbuffer) - ivarint_size(packet_length), temp_out + ivarint_size(packet_length));
            memcpy(conn->outbuffer + ivarint_size(packet_length), temp_out + ivarint_size(packet_length), sizeof(temp_out) - ivarint_size(packet_length));
            ...
            memmove(conn->outbuffer + ivarint_size(packet_length), conn->outbuffer, sizeof(conn->outbuffer) - ivarint_size(packet_length));
    ...
    if (conn->encryption_enabled) {
        aes_encrypt(conn->encryption_ctx, (unsigned char*)conn->outbuffer, (unsigned char*)conn->outbuffer, packet_length + ivarint_size(packet_length));
    }

    // send data
    send(conn->fd, conn->outbuffer, packet_length + ivarint_size(packet_length), 0);
}
...
void read_S01PacketEncryptionRequest(struct Packet* packet, char* buffer) {
    ...
    packet->data.S01PacketEncryptionRequest.server_id = decode_string(temp_offset);
    temp_offset += pstrsize(buffer);

    packet->data.S01PacketEncryptionRequest.pub_key_length = decode_varint(temp_offset);
    ...
    packet->data.S01PacketEncryptionRequest.verify_token_length = decode_varint(temp_offset);
    temp_offset += varint_size(temp_offset);
...
struct Packet receive_packet(char* buffer, struct Connection* conn) {
    ...
    if (conn->state == HANDSHAKING) {
        printf("Something went wrong. Likely forgot to change state away from handshake.");
    } else if (conn->state == PLAY) {
...
void process_S01PacketEncryptionRequest(struct Packet* packet, struct Connection* conn) {
    ...
    SHA1_Update(&ctx, packet->data.S01PacketEncryptionRequest.pub_key, packet->data.S01PacketEncryptionRequest.pub_key_length);
    ...
    cJSON* ret_json;
    if (isNeg) {
        ret_json = join_server(bot.session.accessToken, bot.session.uuid, serv_hash_str);
    } else {
        ret_json = join_server(bot.session.accessToken, bot.session.uuid, serv_hash_str + 1);
    }
    cJSON_Delete(ret_json);

    // asymmetric encryption
    RSA* pub_key = d2i_RSA_PUBKEY(NULL, (const unsigned char**)&packet->data.S01PacketEncryptionRequest.pub_key, packet->data.S01PacketEncryptionRequest.pub_key_length);

    char* shared_enc = malloc(RSA_size(pub_key));
    char* verify_enc = malloc(RSA_size(pub_key));

    RSA_public_encrypt(sizeof(shared_key), (const unsigned char*)shared_key, (unsigned char*)shared_enc, pub_key, RSA_PKCS1_PADDING);
    RSA_public_encrypt(packet->data.S01PacketEncryptionRequest.verify_token_length, (const unsigned char*)packet->data.S01PacketEncryptionRequest.verify_token, (unsigned char*)verify_enc, pub_key, RSA_PKCS1_PADDING);

    struct Packet response_packet = {ENCRYPTION_RESPONSE, .data.C01PacketEncryptionResponse = {RSA_size(pub_key), shared_enc, RSA_size(pub_key), verify_enc}};
...
void process_S02PacketLoginSuccess(struct Packet* packet, struct Connection* conn) {
    conn->state = PLAY;
    bot.thePlayer.name = packet->data.S02PacketLoginSuccess.name;
    bot.thePlayer.uuid = packet->data.S02PacketLoginSuccess.uuid;
    printf("Logged in with name: %s and uuid: %s\n", bot.thePlayer.name, bot.thePlayer.uuid);
    usleep(50 * 1000);
    struct Packet chat_packet = {CHAT_MESSAGE_S, .data.C00PacketChatMessage = {"hello server!"}};
    send_packet(&chat_packet, &bot.conn);
}

void process_S03PacketSetCompression(struct Packet* packet, struct Connection* conn) {
